  ...
  >
    <div className="relative mt-1">
      <ListboxButton className="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm shadow-sm focus:border-indigo-500 focus:ring-2 focus:ring-indigo-400 outline-none">
        {watch.trending === null ? "-- Select --" : watch.trending ? "True" : "False"}
      </ListboxButton>
      <ListboxOptions className="absolute w-full mt-1 bg-white border border-gray-300 rounded-lg shadow-lg z-50">
        {trendOptions.map((option) => (
          ...
          <babel className="flex flex-col gap-2 text-sm font-medium text-gray-700">
            Second Image:
            <input 
            className="block w-full text-sm text-gray-600 file:mr-4 file:rounded-lg file:border-0 file:bg-indigo-50 file:px-4 file:py-2 file:text-indigo-700 hover:file:bg-indigo-100 cursor-pointer"
...
src={URL.createObjectURL(watch.secondImage)} alt="preview" />
          ...
          <button type="submit" className="border rounded-2xl  shadow-black hover:shadow-2xl w-[20%]">Upload</button>
        </form>
      </div>
    </div>
  );
