...
function BuyPage() {
  ...
  async function watchDetail(){
    try{
      ...
      const detail = "Watch id: "+watch!.id+ ", Watch Model: "+watch?.modelNumber+
      ", Watch Brand: "+watch?.brand+", Phone no: "+phoneNumber+", Email id: "+
      emialId+", Address: "+address;
      return detail;
    }catch(error){
      console.log(error);
    }
  ...
  async function handleEmail(): Promise<void> {
    try{      
      const mailBody = {to:API_EMAIL,subject:"Watch Order placed",message: watchDetail()        
      }
      ...
      <div className="flex flex-col gap-6  py-10 items-center">
        <div className="flex flex-col md:w-[40%] w-[80%] border-2 border-gray-400 rounded-2xl py-6 px-4 gap-7 h-[450px">
          ...
          <div className="flex justify-between gap-3">
            ...
            <input className="bg-gray-300 rounded-[5px] py-1  text-center px-2
             text-black font-medium shadow-[5px_5px_8px] shadow-black hover:shadow-lg" placeholder="Enter Your Email id" required/>
          </div>
